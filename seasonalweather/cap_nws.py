...
# Marine SAME FIPS "SS" (digits 1-2 of PSSCCC) that have no state abbreviation.
# Mapped to NWS UGC zone prefixes so they can be appended to the ?area= query,
# which the NWS CAP API accepts alongside state codes (e.g. "MD,ANZ535,ANZ536").
# This is what makes marine alerts (SMW, MWS) visible to the CAP poller.
...
def _derive_area_states(same_fips_allow: Iterable[Any]) -> list[str]:
    """
    Derive NWS CAP ?area= query tokens from the SAME allow list.

    Returns a mix of:
      - State abbreviations (e.g. "MD", "VA") for land counties/zones.
      - Marine UGC zone IDs (e.g. "ANZ535", "ANZ536") for marine zones.

    The NWS CAP API accepts both in a single comma-separated ?area= parameter,
    so including marine zone IDs ensures marine alerts (SMW, MWS) are returned
    alongside land alerts when marine SAME FIPS are in the service area.
    ...
    for raw in same_fips_allow:
        ...
        # Not a land-state FIPS. Check if it's a recognised marine zone SS.
    ...
    # Stable ordering for log comparisons
    states.sort()
    marine_zones.sort()

    # If land-state derivation fails, fall back to the service area's default region.
    if not states:
        states = ["DC", "MD", "VA", "WV"]
